...
%token PROGRAM
%token END
%token<cadena> FIRST_INDEX_OF
%token<cadena> CONCATENAR_CON_RECORTE
%token<cadena> CTE_E
%token<cadena> CTE_R
%token<cadena> ID
...
start:
		...
		PROGRAM programa {printf("Fin del Programa\n"); exportar(); generar_asm();} END ;
;
programa:
...
factor:
		...
		| constante_string;
		| firstIndexOf { printf("R55: factor -> FIRST_INDEX_OF\n"); } ;
		| concatenarConRecorte { printf("R56: factor -> CONCATENAR_CON_RECORTE\n"); } ;
		;
